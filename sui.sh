...
install_s-ui() {
    ...
    # 使用你已上传到 /root 的 s-ui.tar.gz，不再从 GitHub 下载
    if [[ ! -f "/root/s-ui.tar.gz" ]]; then
        echo -e "${red}Error: /root/s-ui.tar.gz not found! Please upload the file first.${plain}"
        exit 1
    fi
    ...
    echo -e "${green}s-ui installation finished, it is up and running now..."
